...
import ResultIntent from "../components/ResultIntent.tsx";

export default function InputText() {
  const [text, setText] = useState("");
  const [url, setUrl] = useState("");

  const { result, counterCharacter } = useGenerator({ text, url });

  return (
    <>
      <section class="flex flex-col items-center justify-center w-full">
        <div class="flex flex-col justify-center mt-8 w-3/4">
          ...
          <label class="text-xl font-bold text-white mt-4">Url</label>
          <input
